...
useEffect(() => {
    const fetchCartItems = async () => {
      try {
        const response = await axios.get('https://proyecto-final-2eg7.onrender.com/cart', {
          headers: {
  ...
  const addToCart = async (movie) => {
    try {
      const response = await axios.post('http://https://proyecto-final-2eg7.onrender.com/cart', { movie }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const removeFromCart = async (movieId) => {
    try {
      await axios.delete(`http://https://proyecto-final-2eg7.onrender.com/cart/${movieId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
